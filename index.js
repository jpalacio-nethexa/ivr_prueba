...
//let  endpointserver = require('./includes/endpointserver');
...
/***
**
** Se busca los datos del servdor para mandarlos al endpoint
**/
var uuid2 = shell.exec('dmidecode | grep UUID', {silent:true}).stdout;
var macs = shell.exec(`ifconfig -a | grep -o -E '([[:xdigit:]]{1,2}:){5}[[:xdigit:]]{1,2}'`, {silent:true}).stdout;
var ips2 = shell.exec(`ifconfig -a | grep -o -E "inet ([0-9]{1,3}[\.]){3}[0-9]{1,3}" | grep -o -E "([0-9]{1,3}[\.]){3}[0-9]{1,3}"`, {silent:true}).stdout;
console.log(uuid2, ips2, macs);


//apiendpoint
endpointserver(uuid2, ips2, macs);
//await sms.sendsms(phone,placa1,drivername1,driversign);
/****
** fin de obtener informacion del servidor
**
**/
function salir(){
	process.exit(1);
...
//             		let valor = await tiempoEspera();
	...
	*/
	function statuscall()
	{
	    ...
	    PhoneNumber=${phone}
	    DialledNumber=${config.diallednumber}
	    BOP=true
	    CLI=false
	    AddResponseID=true
